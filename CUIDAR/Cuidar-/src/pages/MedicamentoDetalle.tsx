...
import { formatearFechaHora24 } from "../utils/formatoFecha";
...
export function MedicamentoDetalle() {
  ...
  const fichaClinica = medicamento ? obtenerFichaMedicamento(medicamento.idMedicamento) : {};
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <section className="mt-5 overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm">
          ...
          <div className="grid gap-4 p-6 sm:grid-cols-2 lg:grid-cols-4 sm:p-8">
            ...
            <Dato icon={<FileText />} label="Tipo" value={medicamento.fechaFin ? `Temporal hasta ${formatearFechaCorta(medicamento.fechaFin)}` : "Crónico / de por vida"} />
          </div>
        </section>
        <section className="mt-6 rounded-3xl border border-gray-200 bg-white p-6 shadow-sm sm:p-8">
          <h2 className="text-2xl font-bold">Ficha clínica del medicamento</h2>
          <p className="mt-2 text-[#747970]">Información de referencia proveniente del catálogo de medicamentos. Ante dudas, consultar siempre con un profesional.</p>
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            <article className="rounded-2xl border border-[#F4B4B4] bg-[#FFF7F7] p-4">
              <p className="font-bold text-[#8A1C1C]">Contraindicaciones</p>
              <p className="mt-2 text-[#747970]">{medicamento.contraindicaciones || fichaClinica.contraindicaciones || "Sin contraindicaciones cargadas."}</p>
            </article>
            <article className="rounded-2xl border border-[#F6D58A] bg-[#FFFDF4] p-4">
              <p className="font-bold text-[#8A5A00]">Efectos secundarios</p>
              <p className="mt-2 text-[#747970]">{medicamento.efectosSecundarios || fichaClinica.efectosSecundarios || "Sin efectos secundarios cargados."}</p>
            </article>
          </div>
        </section>
        <section className="mt-6 rounded-3xl border border-gray-200 bg-white p-6 shadow-sm sm:p-8">
