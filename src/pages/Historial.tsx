...
import { GraficosHistorial } from "../components/GraficosHistorial";
import {
        ...
        <div className="mt-8">
          {cargando ? (
            ...
            <div>
              ...
              <section className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-6" aria-label="Estadísticas del historial">
                ...
                <Estadistica label="Promedio ánimo" value={estadisticas.promedioAnimo} detail={estadisticas.detallePromedioAnimo} icon={<HeartPulse size={20} />} />
                <Estadistica label="Estados bajos" value={`${estadisticas.porcentajeEstadosBajos}%`} detail={estadisticas.detalleEstadosBajos} icon={<TrendingUp size={20} />} />
              </section>

              <GraficosHistorial tomas={tomasFiltradas} animos={animosFiltrados} />
...
function TarjetaMomento({ momento }: { momento: MomentoHistorial }) {
  ...
  const categoria = momento.animo ? categoriaAnimo(momento.animo) : "";
  const motivo = momento.animo ? motivoAnimo(momento.animo) : "";
  const valor = momento.animo ? valorAnimo(momento.animo) : 0;
          ...
          {momento.animo ? (
            <div className="flex h-full items-center gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-lg font-bold">{categoria}</p>
                <p className="mt-1 text-sm font-semibold text-[#386641]">{valor}/5 · {valor * 20}%</p>
                {motivo && <p className="mt-2 text-sm text-[#747970]">{motivo}</p>}
...
function calcularEstadisticas(tomas: RegistroTomaApi[], animos: HistorialAnimoApi[]) {
  ...
  const animosConToma = animos.filter((animo) => tomas.some((toma) => toma.idHistorialAnimo === animo.idHistorialAnimo));
  const basePromedio = animosConToma.length ? animosConToma : animos;
  const valores = basePromedio.map(valorAnimo);
  const promedio = valores.length ? valores.reduce((suma, valor) => suma + valor, 0) / valores.length : 0;
  const estadosBajos = animos.filter((animo) => valorAnimo(animo) <= 2).length;
  ...
  return {
    ...
    animoPredominante,
    promedioAnimo: valores.length ? `${promedio.toFixed(1)}/5` : "—",
    detallePromedioAnimo: valores.length ? `${Math.round((promedio / 5) * 100)}% en ${valores.length} ${valores.length === 1 ? "registro" : "registros"}` : "Sin datos",
    porcentajeEstadosBajos: animos.length ? Math.round((estadosBajos / animos.length) * 100) : 0,
    detalleEstadosBajos: `${estadosBajos} de ${animos.length} ${animos.length === 1 ? "ánimo" : "ánimos"}`,
...
function valorAnimo(animo: HistorialAnimoApi) {
  const categoria = categoriaAnimo(animo);
  if (categoria === "Muy bien") return 5;
  if (categoria === "Bien") return 4;
  if (categoria === "Regular") return 3;
  if (categoria === "Mal") return 2;
  return 1;
}

function motivoAnimo(animo: HistorialAnimoApi) {
  const texto = textoAnimo(animo);
  const categoria = categoriaAnimo(animo);
  const motivo = texto
    .replace(new RegExp(`^${categoria}\\s*[:\\-–—]?\\s*`, "i"), "")
    .trim();
  return motivo && motivo !== categoria ? motivo : "";
}
