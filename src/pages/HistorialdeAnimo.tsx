...
const malestaresFrecuentes = ["Me da acidez", "Me revuelve el estómago", "Tiene mal sabor", "Me da dolor de cabeza", "Me produce náuseas", "Me genera mareos", "Me provoca somnolencia", "Otros"];
...
export function HistorialAnimo() {
  ...
  const [malestares, setMalestares] = useState<string[]>([]);
  ...
  const guardarEstadoAnimo = async () => {
    ...
    if (requiereMotivo && malestares.length === 0 && !observacion.trim()) {
      alert("Contanos brevemente el motivo para poder acompañar mejor ese estado de ánimo.");
      return;
    }
    if (malestares.includes("Otros") && !observacion.trim()) {
      alert("Contanos qué malestar sentiste en la opción Otros.");
      return;
    }

    const idEstado = estadoActual.valor <= 2 ? 3 : estadoActual.valor === 3 ? 2 : 1;
    const observaciones = requiereMotivo
      ? `${estadoActual.texto}: ${[...malestares.filter((item) => item !== "Otros"), observacion.trim()].filter(Boolean).join("; ")}`
      : estadoActual.texto;
              ...
              {estadosAnimo.map((estado) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (estado.valor > 2) { setObservacion(""); setMalestares([]); }
                  }}
            ...
            {requiereMotivo && (
              <>
                ...
                <fieldset className="mt-6">
                  <legend className="text-lg font-bold">¿Qué malestar sentiste? Podés elegir varios.</legend>
                  <div className="mt-3 grid gap-2 sm:grid-cols-2">
                    {malestaresFrecuentes.map((item) => (
                      <label key={item} className="flex items-center gap-3 rounded-xl border border-gray-300 p-3">
                        <input type="checkbox" checked={malestares.includes(item)} onChange={(event) => setMalestares((actual) => event.target.checked ? [...actual, item] : actual.filter((valor) => valor !== item))} />
                        {item}
                      </label>
                    ))}
                  </div>
                ...
                <div className="mt-6">
                  <label htmlFor="detalle-malestar" className="mb-3 block text-lg font-bold md:text-xl">{malestares.includes("Otros") ? "Contanos qué pasó" : "Detalle adicional (opcional)"}</label>
                  <textarea
                    id="detalle-malestar"
                    value={observacion}
                    onChange={(e) => setObservacion(e.target.value)}
                    maxLength={300}
                    required={malestares.includes("Otros")}
