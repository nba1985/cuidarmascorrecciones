...
export interface AlergiaApi { idAlergia: number; descripcion: string }
export interface CondicionApi { idCondicion: number; tipo: string }
...
export interface SolicitudRecuperoPasswordResponse {
  mensaje: string;
}
...
export function obtenerDosisFrecuente(idMedicamento: number) {
  return request<{ dosis: string }>(`/Medicamentos/${idMedicamento}/dosis-frecuente`);
}
...
export function obtenerAlergias() { return request<AlergiaApi[]>("/Alergia"); }
export function obtenerCondiciones() { return request<CondicionApi[]>("/Condiciones"); }
export function obtenerCiudades() { return request<string[]>("/Usuarios/ciudades"); }
...
export function solicitarRecuperoPassword(mail: string) {
  return request<SolicitudRecuperoPasswordResponse>("/Auth/solicitar-recupero", {
    method: "POST",
    body: JSON.stringify({ mail }),
  });
}

export function restablecerPassword(mail: string, token: string, nuevaPassword: string) {
  return request<{ mensaje: string }>("/Auth/restablecer-password", {
    method: "POST",
    body: JSON.stringify({ mail, token, nuevaPassword }),
  });
}
